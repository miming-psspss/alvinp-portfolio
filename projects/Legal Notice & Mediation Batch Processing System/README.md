...
The original **Legal Notice & Mediation Batch Processing System** is a VBA/Excel tool I
built and have run in daily production at Santa Ana Multi-Purpose Cooperative (SAMCO)
since 2022. It's a one-click engine that generates demand letters, account summaries, and
mediation forms for collection cases — logic that had grown organically over years of
direct use and was never formally documented.

The original VBA tool reduced legal collection document generation from hours to under
1 minute per case. This repository is a from-scratch **Python/PySide6 rebuild** of that
system: I reverse-engineered the undocumented VBA logic — the field relationships, the
document generation rules, the mediation form structure — and rebuilt it as a modern
desktop application with a SQLite backend and templated docx generation, instead of an
Excel workbook full of macros. The original `.xlsm` stays private (see the main portfolio
README for why); this is the public, sanitized demonstration of the same underlying
system, rebuilt for portability and maintainability.
